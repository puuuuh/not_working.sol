...
use crate::{error::TypeCheckError, resolver::{resolve_item, TypeResolution}, tys::{common_types, unknown, ElementaryTypes, Ty, TyKind}};
        ...
        match expr.kind(db) {
            Expr::Index { target, index } => {
                ...
                let expected = match target_ty.kind(db) {
                    ...
                    _ => {
                        emit_error(db, target.node(db), format!("can't index into {}", target_ty.pretty_print(db)));
                        return;
                    }
                };
...
fn emit_error<'db, T: rowan::ast::AstNode>(db: &'db dyn BaseDb, node: Option<FileAstPtr<T>>, msg: String) {
