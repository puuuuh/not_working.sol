...
use hir_def::{lower_file, Ident, ImportId, ImportKind, Item};
use indexmap::IndexSet;
use smallvec::SmallVec;
use vfs::{AnchoredPath, File};
use salsa::{plumbing::AsId, Accumulator};
use salsa::Database;
...
fn add_items<'db>(db: &'db dyn BaseDb, import_id: ImportId<'db>, data: &mut BTreeMap<Ident<'db>, SmallVec<[Item<'db>; 1]>>, items: impl IntoIterator<Item = (Ident<'db>, Item<'db>)>) {
...
pub fn resolve_file_root<'db>(db: &'db dyn BaseDb, project: Project, file: File) -> BTreeMap<Ident<'db>, SmallVec<[Item<'db>; 1]>> {
    ...
    for import in f.data(db).imports(db) {
        let Some(mut path) = import.path(db) else {
            NameresError {
                kind: NameresErrorKind::Import(ImportResolutionError::FileNotFound { import_id: import.as_id() })
            }.accumulate(db);
            continue;
        };
        ...
        match import.kind(db) {
            ImportKind::Path { name, .. } => {
                if let Some(name) = name {
                    add_items(db, *import, &mut items, [(name, Item::Module(lower_file(db, file)))]);
                } else {
                    add_items(db, *import, &mut items, resolve_file_root(db, project, file).iter().flat_map(|(name, i)| i.iter().map(|i| (*name, *i))));
                }
            },
            ImportKind::Aliases { symbol_aliases, .. } => {
                ...
                for s in symbol_aliases {
                    ...
                    let Some(item) = root.get(&name) else {
                        NameresError {
                            kind: NameresErrorKind::Import(ImportResolutionError::ItemNotFound { import_id: import.as_id(), name: name.data(db).to_string() })
                        }.accumulate(db);
                        continue;
                    };
...
mod tests {
    ...
    use hir_def::{lower_file, Ident, Item};
    ...
    fn recursive_imports() {
        ...
        let third = lower_file(db, db.file_unchecked(&vfs::VfsPath::from_virtual("/third.sol".into())));
        ...
        let expected: BTreeMap<_, smallvec::SmallVec<[_; 1]>> = BTreeMap::from_iter([
            ...
            (Ident::new(db, "B2"), smallvec![Item::Contract(sec.data(db).contract(db, "B2"))].into()),
            (Ident::new(db, "A1"), smallvec![Item::Contract(main.data(db).contract(db, "A1"))].into()),
        ]);
    ...
    fn simple_imports() {
        ...
        let project = Project::new(&db, vfs::VfsPath::from_virtual("".to_owned()));
        {
            ...
            let expected = BTreeMap::from_iter([
                ...
                (Ident::new(db, "C1"), smallvec![Item::Contract(third.data(db).contract(db, "C1"))]),
                (Ident::new(db, "C2_RENAMED"), smallvec![Item::Contract(third.data(db).contract(db, "C2"))]),
                (Ident::new(db, "A1"), smallvec![Item::Contract(main.data(db).contract(db, "A1"))]),
                (Ident::new(db, "B1"), smallvec![Item::Contract(sec.data(db).contract(db, "B1"))]),
