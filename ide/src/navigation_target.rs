use base_db::BaseDb;
use hir_def::{
    hir::{EnumerationVariantId, HasFile, HasSyntax, Item, StructureFieldId, VariableDeclaration},
    FileExt, InFile,
};
use rowan::ast::AstNode;
use syntax::{
    ast::nodes::{self, ModifierDefinition, Name},
    TextRange,
};
use vfs::File;
...
impl NavigationTarget {
    pub fn from_variant(
        db: &'_ dyn BaseDb,
        d: EnumerationVariantId<'_>,
    ) -> Option<NavigationTarget> {
        let e = d.parent(db);
        let node = e.syntax(db);
        let name = d.name(db).data(db);
        let variable = node.enum_members().find(|e| {
            e.name().and_then(|n| n.ident_token()).map(|n| n.text() == name) == Some(true)
        })?;
        let full_range = node.syntax().text_range();
        let focus_range = variable.syntax().text_range();
        Some(NavigationTarget { file: e.file(db), full_range, focus_range })
    }
    ...
    pub fn from_field(db: &'_ dyn BaseDb, d: StructureFieldId<'_>) -> Option<NavigationTarget> {
        ...
        let name = d.name(db).data(db);
        let variable = node.struct_members().find(|e| {
            e.name().and_then(|n| n.ident_token()).map(|n| n.text() == name) == Some(true)
        })?;
        let full_range = node.syntax().text_range();
        let focus_range = variable.syntax().text_range();
        ...
        let (full_range, focus_range) = match d {
            ...
            Item::Function(function_id) => {
                ...
                let focus = match &node {
                    nodes::FunctionDefinition::NamedFunctionDefinition(
                        named_function_definition,
                    ) => named_function_definition.name().map(name_range),
                    nodes::FunctionDefinition::FallbackFunctionDefinition(
                        fallback_function_definition,
                    ) => fallback_function_definition.fallback_token().map(|t| t.text_range()),
                    nodes::FunctionDefinition::ReceiveFunctionDefinition(
