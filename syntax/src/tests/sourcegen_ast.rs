...
fn sourcegen_ast() {
    ...
    let syntax_kinds_file = sourcegen::project_root().join("syntax/src/syntax_kind/generated.rs");
    sourcegen::ensure_file_contents(syntax_kinds_file.as_path(), &syntax_kinds);

    let grammar =
        include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/solidity.ungram")).parse().unwrap();
    let ast = lower(&grammar);

    let ast_tokens = generate_tokens(&ast);
    let ast_tokens_file = sourcegen::project_root().join("syntax/src/ast/generated/tokens.rs");
    sourcegen::ensure_file_contents(ast_tokens_file.as_path(), &ast_tokens);

    let ast_nodes = generate_nodes(KINDS_SRC, &ast);
    let ast_nodes_file = sourcegen::project_root().join("syntax/src/ast/generated/nodes.rs");
...
fn generate_nodes(kinds: KindsSrc<'_>, grammar: &AstSrc) -> String {
        ...
        .map(|node| {
            ...
            let methods = node.fields.iter().map(|field| {
                ...
                match field {
                    ...
                    Field::Node { cardinality: Cardinality::Optional, .. } => {
                        quote! {
                    ...
                    Field::Token { cardinality: Cardinality::Many, .. } => {
                        let token_kind = field.token_kind().unwrap();
                        quote! {
                            pub fn #method_name(&self) -> #ty {
                                support::tokens(&self.syntax, #token_kind)
                            }
                        }
                    }
                    Field::Token { cardinality: Cardinality::Optional, .. } => {
                        let token_kind = field.token_kind().unwrap();
                        quote! {
                            pub fn #method_name(&self) -> Option<#ty> {
    ...
    {
        eprintln!("Warning: node {} not defined in ast_id source", node);
        drop(node)
    }
    ...
    let ast = quote! {
        ...
        use crate::{
            ...
            ast::{self, AstNode, AstChildren, AstTokenChildren, support},
            T, SolidityLang
        };
...
fn generate_syntax_kinds(grammar: KindsSrc<'_>) -> String {
    ...
    let ast = quote! {
                ...
                if let Some(n) = text.strip_prefix("bytes") {
                    ...
                    if let Some(n) = text.strip_prefix("fixed") {
                        if let Some((m, n)) = n.split_once('x') {
                            if let Ok((m, n)) = m
                                .parse::<u16>()
                                .and_then(|m| n.parse::<u8>().map(|n| (m, n)))
                            {
        ...
        macro_rules! T {
            ...
            [elementary_type_ident] => { $crate::SyntaxKind::ELEMENTARY_TYPE_IDENT };
            [pragma_token] => { $crate::SyntaxKind::PRAGMA_TOKEN };
...
impl Field {
        ...
        match self {
            Field::Token { name, cardinality, .. } => {
                ...
                format_ident!(
                    "{}_token{}",
                    name,
                    if *cardinality == Cardinality::Many { "s" } else { "" }
                )
            }
            Field::Node { name, .. } => {
                if name == "type" {
    ...
    fn ty(&self) -> proc_macro2::Ident {
        match self {
            Field::Token { cardinality: Cardinality::Optional, .. } => format_ident!("SyntaxToken"),
            Field::Token { cardinality: Cardinality::Many, .. } => {
                format_ident!("AstTokenChildren")
            }
...
fn lower(grammar: &Grammar) -> AstSrc {
    let mut res = AstSrc {
        tokens: "Whitespace Comment String HexString DecimalNumber HexNumber PragmaToken Ident"
...
fn lower_rule(acc: &mut Vec<Field>, grammar: &Grammar, label: Option<&String>, rule: &Rule) {
    ...
    match rule {
        ...
        Rule::Token(token) => {
            ...
            let field = Field::Token { name, cardinality: Cardinality::Optional };
            acc.push(field);
        }
        Rule::Rep(inner) => {
            ...
            if let Rule::Token(token) = &**inner {
                let name = grammar[*token].name.clone();
                acc.push(Field::Token { name, cardinality: Cardinality::Many });
                return;
            }
